...
function retVal = read1443Data(fileName)

%%% This script is used to read the binary file produced by the DCA1000 %%% and Mmwave Studio %%% Command to run in Matlab GUI - readDCA1000('<ADC capture bin file>')

 %% global variables % change based on sensor config 

% fileName = 'adc_data444.bin';
...
% read .bin file 
fid = fopen(fileName,'r'); % DCA1000 should read in two's complement data 
adcData = fread(fid, 'int16'); % if 12 or 14 bits ADC per sample compensate for sign extension 
if numADCBits ~= 16
...
%% organize data by LVDS lane % for real only data if isReal
